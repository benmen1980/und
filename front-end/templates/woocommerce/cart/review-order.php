...
<table class="shop_table woocommerce-checkout-review-order-table">
	...
	<tfoot>
        ...
        <?php foreach ( WC()->cart->get_coupons() as $code => $coupon ) :
            //if enter if- that means that we have private purchase amout
            if(($user_budget_left) > $budgets_in_campaign){
                //that means that we already used all the budget
                $coupon_amount = 0;
            }
            else{
                if($private_purchase_amount){
                    if($amount_total > ((int)$budgets_in_campaign - (int)$user_budget_left)){
                        $coupon_amount = (int)$budgets_in_campaign - (int)$user_budget_left;
                    }
                    else{
                        $coupon_amount = $amount_total;
