...
add_filter( 'woocommerce_product_pre_search_products', 'unid_woocommerce_product_pre_search_products', 10, 6 );

function unid_get_valid_search_terms( $terms ) {
	$valid_terms = array();
	$stopwords   = unid_get_search_stopwords();
...
function unid_get_search_stopwords() {
	// Translators: This is a comma-separated list of very common words that should be excluded from a search, like a, an, and the. These are usually called "stopwords". You should not simply translate these individual words into your language. Instead, look for and provide commonly accepted stopwords in your language.
	$stopwords = array_map(
		'wc_strtolower',
		array_map(
...
function unid_woocommerce_product_pre_search_products( $false, $term, $type, $include_variations, $all_statuses, $limit ){
	var_dump('test LIVE'); 
		global $wpdb;
		$post_types   = $include_variations ? array( 'product', 'product_variation' ) : array( 'product' );
		$type_where   = '';
		...
		foreach ( $term_groups as $term_group ) {
			// Parse search terms.
			if ( preg_match_all( '/".*?("|$)|((?<=[\t ",+])|^)[^\t ",+]+/', $term_group, $matches ) ) {
				$search_terms = unid_get_valid_search_terms( $matches[0] );
				$count        = count( $search_terms );
