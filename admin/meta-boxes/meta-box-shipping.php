...
function render_shipping_to_user_brunch($campaign_id) {
	...
	$output = '<fieldset>';
	$output .= '<h4 class="shipping-option-label">' . esc_html__( 'Shipping to user\'s brunch', 'unidress' ) .'</h4>';
	$output .= '<ul class="acf-checkbox-list acf-bl">';
	$output .= '	<li>';
	$output .= '		<label>';
	$output .= '			<input type="checkbox" name="shipping_allow" ' .$checked. '>';
	$output .=              __( 'Allow shipping to user\'s brunch', 'unidress' );
	$output .= '		</label>';
	$output .= '	</li>';
	$output .= '</ul>';
	$output .= '</fieldset>';
...
function save_shipping_option( $post_id ) {
	...
	if ( !isset( $_POST['project_shipping_option'] )
	     || !wp_verify_nonce( $_POST['project_shipping_option'], basename( __FILE__ ) ) )
		return $post_id;
